...
Процедура ВыполнитьСопоставлениеОбъектовИнформационныхБаз(Отказ)
	...
	Для Каждого СтрокаИсточника Из ТаблицаИсточника Цикл
		Для Каждого Поле Из ПользовательскиеПоля Цикл
			Если Не ЗначениеЗаполнено(СтрокаИсточника[Поле]) Тогда
				Продолжить;
			КонецЕсли;
			
			Если Не ОбщегоНазначения.ЭтоСсылка(ТипЗнч(СтрокаИсточника[Поле])) Тогда
				Продолжить;
			КонецЕсли;
			
			Если ОбщегоНазначения.СсылкаСуществует(СтрокаИсточника[Поле]) Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокаИсточника[Поле] = Неопределено;
		КонецЦикла;
	КонецЦикла;
