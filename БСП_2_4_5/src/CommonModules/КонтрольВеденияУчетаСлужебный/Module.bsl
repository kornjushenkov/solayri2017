...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Если Не ПравоДоступа("Просмотр", Метаданные.РегистрыСведений.РезультатыПроверкиУчета) Тогда
		Возврат;
	КонецЕсли;
...
// См. КонтрольВеденияУчета.ЭтоНеразделенныйОбъектМетаданных
...
//
// Параметры:
//   ПоляСинхронизации - Структура - Параметры по которым будет производиться поиск вида проверок:
//       * Свойство1 - ЛюбаяСсылка, Булево, Число, Строка, Дата - Первое свойство синхронизации.
//       * Свойство2 - ЛюбаяСсылка, Булево, Число, Строка, Дата - Второе свойство синхронизации.
//       * Свойство3 - ЛюбаяСсылка, Булево, Число, Строка, Дата - Третье свойство синхронизации.
//                                             ...                                             
//       * СвойствоN - Булево, Число, Строка, Дата - N-ое свойство синхронизации.
//
...
//
Функция РасширенныйПоискВидаПроверки(ПоляСинхронизации, КоличествоСвойств)
	...
	ТекстУсловий           = " Истина ";
	КоличествоПараметров   = ПоляСинхронизации.Количество() - 1;
	...
	Для Индекс = 1 По КоличествоСвойств Цикл
		...
		Значение  = ПоляСинхронизации[Свойство];
	...
	Для ПороговыйИндекс = КоличествоСвойств + 1 По КоличествоПараметров Цикл
		...
		СтруктураПоиска.Вставить(ИмяКолонки, ПоляСинхронизации[ИмяКолонки]);
...
// справочника видов проверок по переданным полям синхронизации при работе с ним.
//
// Параметры:
//     ПоляСинхронизации - Структура - структура с элементами любого ссылочного типа или любого простого типа
//                         (Булево, Строка, Число и Дата). Подробнее см. ОжидаемыеТипыСвойствВидовПроверок().
//                         По переданным полям синхронизации будет осуществлена блокировка данных.
//       * Свойство1 - ЛюбаяСсылка, Булево, Число, Строка, Дата - Первое свойство синхронизации.
//       * Свойство2 - ЛюбаяСсылка, Булево, Число, Строка, Дата - Второе свойство синхронизации.
//       * Свойство3 - ЛюбаяСсылка, Булево, Число, Строка, Дата - Третье свойство синхронизации.
//                                             ...                                              
//       * СвойствоN - ЛюбаяСсылка, Булево, Число, Строка, Дата - N-ое свойство синхронизации.
//
// Возвращаемое значение - БлокировкаДанных  - объект блокировки справочника видов проверки.
//
Функция БлокировкаВидовПроверок(ПоляСинхронизации)
	...
	Если Не (ОбщегоНазначения.ИнформационнаяБазаФайловая()
		Или ПоляСинхронизации.Количество() - 1 > КоличествоСвойств()) Тогда
		Индекс = 1;
		Для Каждого ПараметрПоиска Из ПоляСинхронизации Цикл
...
//                        в случае если поиск не дал результата.
//
Функция ОбычныйПоискВидаПроверки(ПоляСинхронизации, ПоискПоТочномуСоответствию = Истина)
...
// Создает элемент справочника видов проверки по переданным параметрам.
//
// Параметры:
//   ПоляСинхронизации - Структура - Параметры по которым будет производиться поиск вида проверок:
//       * Свойство1 - ЛюбаяСсылка, Булево, Число, Строка, Дата - Первое свойство синхронизации.
...
//       * СвойствоN - Булево, Число, Строка, Дата - N-ое свойство синхронизации.
//
// Возвращаемое значение: СправочникСсылка.ВидыПроверок - Ссылка на созданный элемент справочника.
//
Функция НовыйВидПроверки(ПоляСинхронизации)
	
	НовыйВидПроверки = Справочники.ВидыПроверок.СоздатьЭлемент();
	НовыйВидПроверки.Наименование = ПоляСинхронизации.Наименование;
	КоличествоСвойств    = КоличествоСвойств();
	КоличествоПараметров = ПоляСинхронизации.Количество() - 1;
	...
	Если КоличествоСвойств > КоличествоПараметров Тогда
		Для Индекс = 1 По КоличествоПараметров Цикл
			ИмяСвойства = "Свойство" + Формат(Индекс, "ЧГ=0");
			НовыйВидПроверки[ИмяСвойства] = ПоляСинхронизации[ИмяСвойства];
		КонецЦикла;
	Иначе
		Для Индекс = 1 По КоличествоПараметров Цикл
			ИмяСвойства = "Свойство" + Формат(Индекс, "ЧГ=0");
			...
			Иначе
				ЗаполнитьЗначенияСвойств(НовыйВидПроверки.СвойстваОбъекта.Добавить(),
					Новый Структура("ИмяСвойства, ЗначениеСвойства", ИмяСвойства, ПоляСинхронизации[ИмяСвойства]));
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
//     ПараметрыВыполненияПроверки - Массив - массив структур со значениями:
//       * Свойство1 - ЛюбаяСсылка, Булево, Число, Строка, Дата - Первое свойство синхронизации.
//       * Свойство2 - ЛюбаяСсылка, Булево, Число, Строка, Дата - Второе свойство синхронизации.
//       * Свойство3 - ЛюбаяСсылка, Булево, Число, Строка, Дата - Третье свойство синхронизации.
//                                             ...                                              
//       * СвойствоN - ЛюбаяСсылка, Булево, Число, Строка, Дата - N-ое свойство синхронизации.
//      см. также КонтрольВеденияУчета.ПараметрыВыполненияПроверки.
//
